...
from src.configuration.constants import DATASET_CLASSES
...
class gpt_code(BaseTransformer):
    ...
    def _custom_init(self):
        # config
        self.config = AutoConfig.from_pretrained(self.hparams.model_name_or_path, trust_remote_code=True)
        # tokenizer
        self.tokenizer = AutoTokenizer.from_pretrained(self.hparams.model_name_or_path, trust_remote_code=True)
        # model
        self.model = self._load_model(self.hparams.model_name_or_path, AutoModelForCausalLM, self.config)

        self._set_up(config=self.config, tokenizer=self.tokenizer, model=self.model)
        self.train_dataset_class = DATASET_CLASSES[self.hparams.training_stage]
        self.test_dataset_class = DATASET_CLASSES[self.hparams.training_stage]
    ...
    def training_step(self, batch, batch_idx) -> Dict:
        ...
        if torch.isnan(loss):
            print(batch)
        self.log("train_loss", loss, prog_bar=True, sync_dist=True)  # metrics logged can be access by trainer.callback_metrics
        logs = {"loss": loss.item(), "batch_size": batch["input_ids"].shape[0]}
    ...
    def test_epoch_end(self, outputs):
        ...
        metric_val = (torch.tensor(loss_metrics[self.val_metric]))
    ...
    def get_dataset(self, data_type):
        if data_type == 'train':
            dataset = self.train_dataset_class(self.hparams, self.tokenizer, data_type)
        else:
            dataset = self.test_dataset_class(self.hparams, self.tokenizer, data_type)
    ...
    def train_dataloader(self) -> DataLoader:
        ...
        return self.get_dataloader("train", batch_size=self.hparams.train_batch_size, shuffle=train_shuffle)
